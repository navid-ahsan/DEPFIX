...
for i in "${!CONTAINER_NAMES[@]}"; do
  ...
  if [[ "$NAME" == "pgvector" ]]; then
    docker run --gpus "$GPU_DEVICES_ARRAY" --cpus="$CPUS" --memory="$MEMORY" --name "$NAME" -p $PORT $ARGS -d $IMAGE
